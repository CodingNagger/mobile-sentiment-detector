...
using MobileSentimentDetector.Model.Extensions;

namespace MobileSentimentDetector.Model
{
    // replicate tests from https://github.com/NMZivkovic/BertTokenizers/blob/master/tests/BertLargeTokenizerUncasedShould.cs
    public class MobileBertTokenizer
    {
        protected readonly List<string> _vocabulary;
        protected readonly Dictionary<string, int> _vocabularyDict;

        public MobileBertTokenizer(string vocabularyResourceName)
        {
            _vocabulary = VocabularyReader.ReadResource(vocabularyResourceName);
        ...
        public MobileBertTokenizer():this("MobileSentimentDetector.Model.Assets.vocab.txt") {
        }
